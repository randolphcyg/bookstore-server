...
func (m *ManageUserService) LockUser(idReq request.IdsReq, lockStatus int) (err error) {
	...
	err = global.DB.Model(&manage.MallUser{}).Where("user_id in ?", idReq.Ids).Update("locked_flag", lockStatus).Error
	return err
}
...
func (m *ManageUserService) DelUser(idReq request.IdsReq, delStatus int) (err error) {
	...
	//更新字段为0时，不能直接UpdateColumns
	err = global.DB.Model(&manage.MallUser{}).Where("user_id in ?", idReq.Ids).Update("is_deleted", delStatus).Error
	return err
}
...
func (m *ManageUserService) GetMallUserInfoList(info manageReq.MallUserSearch) (err error, list interface{}, total int64) {
	...
	// 创建db
	db := global.DB.Model(&manage.MallUser{})
